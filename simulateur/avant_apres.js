...
const SC=[
  ...
  {n:'stress pont 8 mai', y:2027, o:(()=>{const ids=H.defaultRoster().map(r=>r[0]).filter(id=>!['MAR04','MAR17','MAR01','MAR11'].includes(id)).slice(0,15);const im={};ids.forEach(id=>{im[id]={};['2027-05-06','2027-05-07','2027-05-08','2027-05-09','2027-05-10'].forEach(d=>im[id][d]='INDISPO');});return {indisposMap:im};})()},
  {n:'TP fixes mer/jeu', y:2027, o:{roster:H.defaultRoster().map(r=>r[0]==='MAR03'?['MAR03',80,80,{tpJours:'MER,JEU'}]:r)}},
  {n:'maternité CL 2 mois', y:2027, o:(()=>{const im={MAR03:{}};for(let m=1;m<=2;m++)for(let d=1;d<=31;d++){const dt=new Date(2027,m-1,d);if(dt.getMonth()!==m-1)continue;im.MAR03[`2027-0${m}-${String(d).padStart(2,'0')}`]='CL';}return {indisposMap:im};})()},
];
...
const rp=runWith('/home/claude/repo-patched/gas/generateur_gardes.gs',2027,{roster:H.defaultRoster().map(r=>r[0]==='MAR03'?['MAR03',80,80,{tpJours:'MER,JEU'}]:r)});
const Pp=A.parsePlanning(rp.ss,2027);
const dd=Pp.byDoc['MAR03']||{};
const viol=Object.keys(dd).filter(d=>(dd[d]==='G'||dd[d]==='G2')&&[3,4].includes(A.DOW(d)));
const s=H.readStats(rp.ss,2027).byId['MAR03'];
console.log(`\nD1 corrigé : gardes mer/jeu de MAR03 = ${viol.length} (avant patch : 7) | son total=${s['TOTAL G']} cible=${String(s['CIBLE']).replace("'",'')}`);
