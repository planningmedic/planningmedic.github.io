...
function absences(year, roster, seed){
  ...
  jours.forEach(d=>{
    ...
    if(map['MAR01'][d]) return;                 // ses propres congés — la seule exclusion
    if(F.has(d)||F.has(add1(d))) return;          // férié ou veille de férié
    map['MAR01'][d]='SOUHAIT'; n++;
  });
...
const roster = H.defaultRoster().map(r=>{
  const c=[...r]; c[3]={...c[3]};
  if(c[0]==='MAR10') delete c[3].dateDebut;      // présent toute la période
  return c;
});
