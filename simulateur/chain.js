...
[2027,2028,2029,2030].forEach(y=>{
  ...
  console.log(`${y} : MAR02 sam=${s['MAR02']['SAM']} MAR08 sam=${s['MAR08']['SAM']} (cible ${s['MAR02']['CIBLE SAM']}) — écart max sam=${Math.max(...Object.keys(s).filter(id=>+s[id]['TOTAL G']>0&&!['MAR01'].includes(id)).map(id=>Math.abs(+s[id]['SAM']-+s[id]['CIBLE SAM']))).toFixed(2)}`);
  if(inject){ st.forEach(r=>{ if(r[0]==='MAR02') r[iSam]=+r[iSam]+2; if(r[0]==='MAR08') r[iSam]=Math.max(0,+r[iSam]-2); }); inject=false; console.log('   (injection +2/−2 sam dans les stats transmises)'); }
  prev=st;
});
...
console.log(`\nS13 — 18h : min=${Math.min(...h18)} max=${Math.max(...h18)} | MAR04(only18?)=${s['MAR04']?s['MAR04']['18H']:'-'} MAR17=${s['MAR17']?s['MAR17']['18H']:'-'}`);
