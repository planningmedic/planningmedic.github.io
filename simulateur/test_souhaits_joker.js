...
{
  const im = { MAR02: {} };
  jeudisOrd.filter((d, i) => i % 6 === 0).slice(0, 6).forEach(d => im.MAR02[d] = 'SOUHAIT');
  const o = run(`T1 — MAR02 souhaite ${Object.keys(im.MAR02).length} jeudis répartis sur l'année`, im);
  const hon = Object.keys(im.MAR02).filter(d => aGarde(o.P, 'MAR02', d)).length;
  const reel = num(o.st.MAR02['JEU']), cible = num(o.st.MAR02['CIBLE JEU']);
  console.log(`   honorés ${hon}/${Object.keys(im.MAR02).length} | jeudis ${reel} pour une part de ${cible}`);
...
// ── T2 : un souhait de VENDREDI emporte le dimanche (week-end entier) ────────
{
  ...
  const im = { MAR08: { [ven]: 'SOUHAIT' } };
  const o = run(`T2 — MAR08 souhaite le vendredi ${ven} → week-end entier`, im);
  const aVen = aGarde(o.P, 'MAR08', ven), aDim = aGarde(o.P, 'MAR08', dim);
...
// ── T3 : un souhait de DIMANCHE remonte au vendredi ─────────────────────────
{
  ...
  const im = { MAR14: { [dim]: 'SOUHAIT' } };
  const o = run(`T3 — MAR14 souhaite le dimanche ${dim} → le vendredi vient avec`, im);
  ok(o.errs === 0, 'invariants intacts');
  ok(aGarde(o.P, 'MAR14', ven) === aGarde(o.P, 'MAR14', dim),
...
if (lundisFeries.length) {
  ...
  const im = { MAR19: { [lun]: 'SOUHAIT' } };
  const o = run(`T4 — MAR19 souhaite le lundi férié ${lun} → samedi ${sam} couplé`, im);
  ok(o.errs === 0, 'aucun couplage brisé (défaut corrigé)');
  ok(aGarde(o.P, 'MAR19', lun) === aGarde(o.P, 'MAR19', sam),
...
{
  const im = { MAR06: {} };
  samedisOrd.forEach(d => im.MAR06[d] = 'SOUHAIT');           // il les demande TOUS
  const o = run(`T5 — MAR06 souhaite les ${samedisOrd.length} samedis de l'année`, im);
  const hon = samedisOrd.filter(d => aGarde(o.P, 'MAR06', d)).length;
  const reel = num(o.st.MAR06['SAM']), cible = num(o.st.MAR06['CIBLE SAM']);
...
{
  const im = { MAR15: {} };
  vendredisOrd.forEach(d => im.MAR15[d] = 'SOUHAIT');
  const o = run(`T6 — MAR15 souhaite les ${vendredisOrd.length} week-ends de l'année`, im);
  const reel = num(o.st.MAR15['VD']), cible = num(o.st.MAR15['CIBLE VD']);
...
{
  const im = { MAR05: {} };
  jours.forEach(d => im.MAR05[d] = 'SOUHAIT');
  const o = run(`T7 — MAR05 souhaite les ${jours.length} jours de 2027`, im);
  const tot = num(o.st.MAR05['TOTAL G']);
  const cib = parseFloat(String(o.st.MAR05['CIBLE']).replace("'", ''));
  console.log(`   total ${tot} pour une part de ${cib} | samedis ${o.st.MAR05['SAM']} `
    + `week-ends ${o.st.MAR05['VD']} jeudis ${o.st.MAR05['JEU']}`);
  ok(o.errs === 0, 'invariants intacts');
  ok(tot - cib <= 2 + 1e-9, `son total ne dépasse pas sa part de plus de 2 (écart ${(tot - cib).toFixed(1)})`);
  ['SAM', 'JEU', 'VD'].forEach(ax => {
    const c = num(o.st.MAR05['CIBLE ' + (ax === 'VD' ? 'VD' : ax)]);
    ok(num(o.st.MAR05[ax]) <= Math.ceil(c) + 1, `axe ${ax} non monopolisé`);
  });
}
