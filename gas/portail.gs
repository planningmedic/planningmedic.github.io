...
/* ════════════════════════════════════════════════════════════════════
   ...
   ⚠️ AUCUNE ÉCRITURE PAR LE CODE. le responsable recopie le relevé À LA MAIN dans
...
/* À EXÉCUTER DEPUIS L'ÉDITEUR APPS SCRIPT (liste déroulante -> Exécuter) pour créer
   l'onglet du relevé. Sans argument : le responsable n'a rien à taper.
...
/* ════════════════════════════════════════════════════════════════════
   ...
   urgence au service (regle le responsable, 27/07). Le modificateur 7 (presence permanente de
...
function deleteCotationType(payload, user) {
  if (!_cotTypeMembre_(user)) return { success: false, error: 'Réservé aux membres du groupement libéral.' };
  /* SUPPRESSION RESERVEE (decision du responsable, 17/08/2026). La bibliotheque est
...
// ouvrir l'onglet des codes d'acces (decision le responsable, 01/08/2026).
...
function _migrerColonnesXL_(sh) {
  try {
    ...
    for (let r = 1; r < rows.length; r++) {
      ...
      if (!code || !vals) continue;          // secteur créé par le responsable → défauts de getSecteurs
      for (let k = 0; k < 3; k++) {
