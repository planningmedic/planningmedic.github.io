...
| `reprise.html` | **Continuité** : accès, propriété, sauvegardes, réparations — à lire si le responsable n'est plus joignable |
...
`code.gs`, `Indispos.gs` (le routeur : doGet/doPost, checkCode, la table des 64 actions), `gardes.gs`, `indisponibilites.gs`, `temps_partiel.gs`, `equipe.gs`, `diagnostic.gs` (15/09/2026 : les fonctions métier sorties d'Indispos.gs, un fichier par sujet), `generateur_gardes.gs`, `setup_annee.gs`, `portail.gs`, `miroir.gs`, `journal.gs`, `veille.gs`, `echanges.gs` + `appsscript.json`. **`dev.gs` ne se recopie pas** (14/09/2026) : douze fonctions de test manuel sorties des fichiers de production, gardées dans le dépôt pour un développeur, jamais déployées. **Deux fichiers de l'éditeur vivent ailleurs** : `sauvegarde.gs` (racine du dépôt) et `dispo_jour.gs` (= `partage/dispo_jour.js`). Soit **11 fichiers** dans l'éditeur — carte détaillée dans `gas/README.md`.
