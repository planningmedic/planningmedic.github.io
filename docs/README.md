...
`code.gs`, `Indispos.gs`, `generateur_gardes.gs`, `setup_annee.gs`, `portail.gs`, `miroir.gs`, `journal.gs`, `veille.gs` + `appsscript.json`. **`dev.gs` ne se recopie pas** (14/09/2026) : douze fonctions de test manuel sorties des fichiers de production, gardées dans le dépôt pour un développeur, jamais déployées. **Deux fichiers de l'éditeur vivent ailleurs** : `sauvegarde.gs` (racine du dépôt) et `dispo_jour.gs` (= `partage/dispo_jour.js`). Soit **11 fichiers** dans l'éditeur — carte détaillée dans `gas/README.md`.
