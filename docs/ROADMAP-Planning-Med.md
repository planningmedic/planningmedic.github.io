...
la liste `DOCTORS` (25 noms), `DVI_ALLOWED` (3 noms), les tests `id === 'MAR01'` (15 occurrences),
...
| avant | après |
|---|---|
| `only:'MAR24'` sur 5 tuiles + `CRH_ALLOWED` / `STATS_ALLOWED` côté serveur | `CONFIG / TUILES_PRIVEES`, lue par l'écran **et** par le serveur |
| `DVI_ALLOWED = [3 noms]` | `CONFIG / DVI_HABILITES` |
| `id === 'MAR01' ? 'Pr ' : 'Dr '` et `wish: id === 'MAR01'` | listes `titresPr` et `souhaitsPlafond`, déduites des colonnes NOM et `souhait_plafond` |
...
1. **Le calcul à blanc ne prédit pas la génération réelle quand l'effectif n'est pas trié.**
   ...
   directement la génération 2027 : l'onglet MEDECINS n'est pas trié (MAR12, MAR01, MAR11
...
3. **Un souhait posé sur un jour interdit par le profil compte comme non honoré.** Un profil
   ...
   ailleurs disparaître pour qui n'a ni garde ni temps partiel à poser (MAR04, MAR17).
