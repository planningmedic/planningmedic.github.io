...
M.set('acces', JSON.stringify({ users: [
  ...
  { h: await sha256hex(CODE_MAR), role: 'mar', id: 'MAR04' },
]}));
...
// dépôt admin
let r = await appel('/ecrire', { code: CODE_ADMIN, intention: { type: 'placements', year: 2027, items: [{ date: '2027-03-03', marId: 'MAR05', morning: 'MAT' }] } });
...
console.log('\n═══ Lu/★ par MAR : la clé veille_marques, filtrée POUR TOUS ═══');
{
  M.set('veille_marques', JSON.stringify({ parMar: {
    MAR04:   { lus: ['101', '102'], stars: ['103'] },
    DURAND: { lus: ['201'],        stars: [] },
    MAR05: { lus: ['301'],        stars: ['301'] },
  }, t: Date.now() }));
  ...
  V('il ne reçoit QUE ses marques (MAR04 seul)', vmM && Object.keys(vmM.parMar).length === 1 && !!vmM.parMar.MAR04, vmM && Object.keys(vmM.parMar));
  V('ses lus et ses étoiles sont complets', vmM && vmM.parMar.MAR04.lus.length === 2 && vmM.parMar.MAR04.stars[0] === '103', vmM && vmM.parMar.MAR04);
  V('les marques de MAR05 ne fuient pas', vmM && !vmM.parMar.MAR05);
  ...
  V('l\'admin est filtré LUI AUSSI (DURAND seul, jamais les autres)', vmA && Object.keys(vmA.parMar).length === 1 && !!vmA.parMar.DURAND && !vmA.parMar.MAR04, vmA && Object.keys(vmA.parMar));

  // Poussée par le GAS : la clé est acceptée à l'écriture /push
  r = await appel('/push', { token: 'JETON-SECRET', items: { veille_marques: JSON.stringify({ parMar: { MAR04: { lus: ['999'], stars: [] } }, t: 1 }) } });
  V('le GAS peut pousser veille_marques', r.j.success === true, r.j);
  r = await appel('/read', { code: CODE_MAR, keys: ['veille_marques'] });
  V('la nouvelle valeur est servie, toujours filtrée', r.j.data.veille_marques.parMar.MAR04.lus[0] === '999', r.j.data.veille_marques);
}
