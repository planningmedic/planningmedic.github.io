...
console.log('\n═══ 20. Un humain lit ces messages : « Dr Durand », jamais « DURAND » ═══');
{
  ...
  // Le cas particulier de l'écran : MAR01 est Pr, pas Dr
  const dash = fs.readFileSync(path.join(__dirname, '..', 'dashboard.html'), 'utf8');
  V('l\'écran traite MAR01 en « Pr »', /id==='MAR01'\?'Pr':'Dr'/.test(dash.replace(/\s/g, '')), 'règle de _meName');
  const gs = fs.readFileSync(path.join(__dirname, '..', 'gas', 'echanges.gs'), 'utf8');
  V('les notifications suivent LA MÊME règle pour MAR01', /MAR01' \? 'Pr ' : 'Dr '/.test(gs));
}
