/* ═══ BANC — LA PAGE FACE AUX PANNES ═══
   Les trois peurs du responsable : « et si le réseau lâche ? », « et si je ferme la
...
(async () => {
  ...
  console.log('\n═══ 55. Panneau Modifications : gardes par le miroir, échec jamais mémorisé ═══');
  {
    ...
    const { w } = await page(async (url, opt) => {
      if (String(url).includes('workers.dev')) {
        ...
        return { ok:true, json: async () => ({ success:true, identite:{role:'admin'},
          data: { gardes_2027: { success:true, data: { '2027-01-04': { MAR09:'G', MAR21:'G2' } } } } }) };
      }
      appelsGAS++;
    ...
    V('MAR09 y est bien de garde', r1['2027-01-04'].MAR09 === 'G', r1['2027-01-04']);
    ...
    w2.fetch = async (url) => {
      if (String(url).includes('workers.dev')) { revenu = true;
        return { ok:true, json: async () => ({ success:true, identite:{role:'admin'},
          data: { gardes_2027: { success:true, data: { '2027-01-04': { MAR09:'G' } } } } }) }; }
      return { ok:true, json: async () => ({ success:true, data:{} }) };
    };
...
{
  const c = fs.readFileSync('../admin.html', 'utf8');
  /* (23/08/2026) Décision du responsable : un seul canal. Le mail au comité est
