...
async function page(fichier) {
  ...
  const dom = new JSDOM(fs.readFileSync(fichier, 'utf8'), {
    ...
    beforeParse(w) {
      w.eval(require('fs').readFileSync(require('path').join(__dirname, '..', 'partage', 'portail.js'), 'utf8')); w.eval(require('fs').readFileSync(require('path').join(__dirname, '..', 'partage', 'rendu_equite.js'), 'utf8'));   // (14/09/2026) le socle commun, comme le vrai site
...
(async () => {
  ...
  {
    /* (23/08/2026) Décision du responsable : la traçabilité ne se fait qu'à
