...
(async () => {
  ...
  w.queueOverride('2027-03-03','MAR05','am','MAT','Comité — MAT AM');
  w.queueOverride('2027-03-03','MAR05','pm','MAT','Comité — MAT PM');
  w.queueOverride('2027-03-04','MAR06','am','REA','Comité — REA AM');
  ...
  V('MAR05 03/03 matin ET après-midi', lignes.some(l => dstr(l[0])==='2027-03-03' && l[1]==='MAR05' && l[2]==='MAT' && l[3]==='MAT'), lignes[1]);
  ...
  // (c) changement de statut : MAR06 passe en TP le 04/03
  const r = vm.runInContext("appliquerStatutJour(2027,'MAR06','TP',['2027-03-04'])", gctx);
  V('statut TP posé', r.applied.length === 1, r);
  V('le placement MAR06 du 04/03 a disparu tout seul',
    !cl.getSheetByName('PLANNING_OVERRIDES').lignes.some(l => dstr(l[0])==='2027-03-04' && l[1]==='MAR06'),
    cl.getSheetByName('PLANNING_OVERRIDES').lignes.map(l=>l.slice(0,2)));
  V('les placements de MAR05 sont intacts',
    cl.getSheetByName('PLANNING_OVERRIDES').lignes.some(l => l[1]==='MAR05'));

  // (d) réquisition : replacer MAR06 APRÈS le TP
  w.queueOverride('2027-03-04','MAR06','am','REA','Comité — réquisition');
  ...
  w.queueOverride('2027-03-01','MAR10','am','END','Comité — END AM');
  ...
  V('APRÈS FERMETURE : le placement final est au classeur',
    cl.getSheetByName('PLANNING_OVERRIDES').lignes.some(l => l[1]==='MAR10' && l[2]==='END'),
