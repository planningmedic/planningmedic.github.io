...
console.log('\n═══ 1. Le dernier geste gagne (cas MAR06) ═══');
{
  const cl = new Classeur();
  cl.ajouter('PLANNING_OVERRIDES', [
    ['DATE','MAR_ID','MATIN','APREM','COMMENTAIRE'],
    ['2027-03-03','MAR05','MAT','MAT','Comité — MAT'],
    ['2027-03-03','MAR06','REA','REA','Comité — REA'],
    ['2027-03-04','MAR06','REA','REA','Comité — REA'],
  ]);
  ...
  const res = vm.runInContext("appliquerStatutJour(2027, 'MAR06', 'TP', ['2027-03-03'])", ctx);
  const lignes = cl.getSheetByName('PLANNING_OVERRIDES').lignes;
  verifier('statut TP appliqué', res.applied.length === 1, res);
  verifier('le placement MAR06 du 03/03 est retiré',
    !lignes.some(l => l[0] === '2027-03-03' && l[1] === 'MAR06'), lignes.map(l => l.slice(0,2)));
  verifier('le placement MAR06 du 04/03 est INTACT (autre jour)',
    lignes.some(l => l[0] === '2027-03-04' && l[1] === 'MAR06'));
  verifier('le placement MAR05 du 03/03 est INTACT (autre MAR)',
    lignes.some(l => l[0] === '2027-03-03' && l[1] === 'MAR05'));
  verifier('la feuille garde bien 3 lignes + entête', lignes.length === 3, lignes.length);

  // Réquisition : replacer APRÈS le TP
  cl.getSheetByName('PLANNING_OVERRIDES').appendRow(['2027-03-03','MAR06','REA','REA','Comité — réquisition']);
  const res2 = vm.runInContext("appliquerStatutJour(2027, 'MAR06', '18', ['2027-03-03'])", ctx);
  ...
  verifier('« 18 » ne retire PAS le placement (pas une absence)',
    cl.getSheetByName('PLANNING_OVERRIDES').lignes.some(l => dstr(l[0]) === '2027-03-03' && l[1] === 'MAR06'), res2);
}
...
console.log('\n═══ 4. L\'applicateur : ordre, groupage, échec isolé ═══');
{
  ...
  cl.ajouter('GARDES_2027', [['','',''],['','',''],['MAR','','2027-01-12','2027-01-13'],['MAR10','','',''],['MAR05','','','']]);
  ...
  ctx.UrlFetchApp = { fetch: (url, opt) => {
    ...
    if (url.endsWith('/tirer')) return { getResponseCode: () => 200, getContentText: () => JSON.stringify({ success: true, fiches: [
      ...
      { cle: 'j_00000000000010_a', valeur: { type: 'placements', year: 2027, items: [{ date:'2027-01-12', marId:'MAR05' }] } },
      { cle: 'j_00000000000050_e', valeur: { type: 'statut', year: 2027, marId: 'FANTOME', statut: 'V', dates: ['2027-02-01'] } },
      { cle: 'j_00000000000020_b', valeur: { type: 'statut', year: 2027, marId: 'MAR10', statut: 'F', dates: ['2027-01-12'] } },
      { cle: 'j_00000000000040_d', valeur: { type: 'placements', year: 2027, items: [{ date:'2027-01-13', marId:'MAR04' }] } },
  ...
  const iBatch1 = trace.indexOf('batch:MAR05'), iPub = trace.indexOf('publier:2027'), iBatch2 = trace.indexOf('batch:MAR04');
  ...
  verifier('le statut valide est appliqué', trace.includes('statut:MAR10'));
...
console.log('\n═══ 5. Rejeu d\'un lot : mise à jour, jamais de doublon ═══');
{
  ...
  const items = JSON.stringify([{ date:'2027-03-03', marId:'MAR05', morning:'MAT', comment:'Comité' }]);
  ...
  vm.runInContext(`savePlanningOverridesBatch([{date:'2027-03-03',marId:'MAR05',afternoon:'REA'}])`, ctx);
