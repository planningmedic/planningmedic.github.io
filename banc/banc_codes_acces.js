...
function monde(opts) {
  ...
  cl.ajouter('MEDECINS', [
    ...
    ['MAR07', 'DR MAR07', 'RM', 'O', 100, 100, 'ANCIENRM', opts.sansEmail ? '' : 'rm@exemple.mc'],
...
{
  const b = monde();
  const avant = b.codeDe('MAR07');
  const r = b.appel('MAR07');
  V('réponse en succès', r.success === true, r);
  V('le nom du MAR revient à l\'écran', r.nom === 'DR MAR07', r);
  V('le code a bien changé', b.codeDe('MAR07') !== avant, b.codeDe('MAR07'));
  V('le nouveau code respecte l\'alphabet', /^[A-Z2-9]{8}$/.test(b.codeDe('MAR07')), b.codeDe('MAR07'));
  ...
  V('l\'email porte le NOUVEAU code', b.envois[0] && b.envois[0].__code === b.codeDe('MAR07'), b.envois[0] && b.envois[0].__code);
...
console.log('\n═══ C003 · jamais deux personnes avec le même code ═══');
{
  ...
  V('le code reste INCHANGÉ', b.codeDe('MAR07') === avant, b.codeDe('MAR07'));
...
}
{
  ...
  const r = b.appel('MAR07');
  V('le premier code libre est retenu', r.success === true && b.codeDe('MAR07') === 'LIBRE234', b.codeDe('MAR07'));
}
...
console.log('\n═══ C004 · sans email, on ne change RIEN ═══');
{
  ...
  V('le motif nomme le MAR', /MAR07/.test(r.error || ''), r.error);
  V('le code reste INCHANGÉ', b.codeDe('MAR07') === 'ANCIENRM', b.codeDe('MAR07'));
}
...
console.log('\n═══ C005 · email en panne : le dire franchement ═══');
{
  ...
  V('le nouveau code est donné à l\'admin', (r.error || '').indexOf(b.codeDe('MAR07')) >= 0, r.error);
  V('le classeur porte bien le nouveau code', b.codeDe('MAR07') !== 'ANCIENRM');
  V('l\'échec est tracé avec le nouveau code', b.journal.some(l => /ECHEC EMAIL/.test(l) && l.indexOf(b.codeDe('MAR07')) >= 0), b.journal);
}

console.log('\n═══ C006 · réservé au comité, et médecin inconnu refusé ═══');
{
  const b = monde();
  const r = b.appel('MAR07', 'mar');
  V('un MAR ne peut pas réinitialiser un code', r.success === false, r);
  V('le code reste INCHANGÉ', b.codeDe('MAR07') === 'ANCIENRM');
  ...
  V('aucune écriture', b3.codeDe('MAR07') === 'ANCIENRM');
}
